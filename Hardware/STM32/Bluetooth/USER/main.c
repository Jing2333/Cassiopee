...
/*	
		...
		delay(5 second)
		Bluetooth_poll();
			Recuperer la liste des dispositifs (FLASH) - peut etre dans un fichier (FATFS)
			Demander aux dispositifs par ordre
			Stocker les resultats dans un fichier dedie
		}
		
		INT_TCP{
			Recevoir la demande
				Si demande d'ajout
					Recuperer toutes les info dans unu structure puis l'enregistrer dans FLASH (ou le fichier de dispositifs)
					Repondre OK/NOK
				Si demande de l'info d'un dispositif
					Recuperer toutes les info dans son fichier
					Envoyer le
					Supprime?
		}
